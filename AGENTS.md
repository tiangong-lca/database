...
- V2 empty-query, geography-only Flow Search retains its narrow Facet fast path. V3 query-free Process/Flow Search uses the existing navigation and facet projections for filtering and page selection; relevance/date sorts hydrate only the selected page, while name ordering still reads matched names. V3 filtered Facets and navigation counts reuse those narrow facts. Cutover verifies complete, consistent coverage; no index, relation, Trigger, writer or timeout is added. See `docs/agents/repo-architecture.md` and `docs/agents/repo-validation.md` for the read and proof boundaries.
